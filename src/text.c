...
bool badge_set_value(struct badge* badge, char* string) {
  ...
  bool changed = badge->drawing != drawing;
  badge->drawing = drawing;
  return badge_set_string(badge, string, false) || changed;
}
...
bool text_parse_sub_domain(struct text* text, FILE* rsp, struct token property, char* message) {
  ...
  if (token_equals(property, PROPERTY_COLOR)) {
    ...
    return badge_set_value(&text->badge, token_to_string(get_token(&message)));
